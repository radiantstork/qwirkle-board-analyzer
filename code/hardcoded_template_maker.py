...
def get_number_templates():
    ...
    # hardcode area properties to filter out unnecessary contours
    ...
    for contour in contours:
        ...
        x_right = min(img.shape[1], x + w + padding)
        y_bottom = min(img.shape[0], y + h + padding)

        template = img[y_top:y_bottom, x_left:x_right]
...
def get_contours_for_specific_color(board, lower, upper):
    ...
    contours, _ = cv.findContours(mask, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
    return mask, [c for c in contours if cv.contourArea(c) >= 7000]
...
def get_shape_templates():
    ...
    board = cv.imread("aux_imgs/result/08.jpg")
    ...
    for piece_set_info in hardcoded_colors_and_coordinates:
        ...
        mask, contours = get_contours_for_specific_color(smaller_board, lower, upper)
